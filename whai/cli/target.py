...
# Common shell prompt endings used to detect command completion
PROMPT_ENDINGS = ('$', '#', '>', '%', '»', '❯', '➜')


def _run_tmux(args: List[str]) -> Optional[subprocess.CompletedProcess]:
    """Run a tmux command, returning the result or None on failure."""
    try:
        ...
        logger.debug("tmux command failed (%s): %s", type(exc).__name__, exc)
        return None
...
def pane_exists(pane_id: str) -> bool:
    ...
    """
    result = _run_tmux(["display-message", "-t", pane_id, "-p", "#{pane_id}"])
...
def capture_target_context(pane_id: str, scrollback_lines: int = TMUX_SCROLLBACK_LINES) -> Optional[str]:
    ...
    """
    result = _run_tmux(["capture-pane", "-t", pane_id, "-p", "-S", f"-{scrollback_lines}"])
    if result is None:
        logger.error(f"tmux command failed for pane {pane_id}")
        return None
...
def send_command_to_target(pane_id: str, command: str) -> bool:
    ...
    """
    result = _run_tmux(["send-keys", "-t", pane_id, command, "Enter"])
    if result is None:
        logger.error(f"tmux command failed for pane {pane_id}")
        return False
    if result.returncode == 0:
