...
def test_malformed_tool_call_json_recovers_gracefully():
    ...
    with (
        ...
        assert (
            "error" in output.lower() or "invalid" in output.lower() or len(output) > 0
        )
...
def test_network_failure_shows_retry_message():
    ...
    with (
        ...
        assert (
            "error" in output.lower() or "failed" in output.lower() or len(output) > 0
        )
...
def test_very_large_command_output_truncated():
    ...
    with (
        patch("subprocess.Popen", return_value=FakeProcess(large_output)) as mock_popen,
