...
def test_lm_studio_validate_model_found():
    """Test LM Studio model validation when model is found."""
    import json
    ...
    # Mock API response with the model in the list
    mock_response_data = {
        "data": [
            {"id": "codestral-22b-v0.1", "object": "model"},
    ...
    mock_response.read.return_value.decode.return_value = json.dumps(mock_response_data)

    with patch("urllib.request.urlopen", return_value=mock_response):
        model_valid, model_issue = lm_config._validate_model()

    assert model_valid is True
    assert model_issue is None


def test_lm_studio_validate_model_not_found():
    """Test LM Studio model validation when model is not found."""
    import json
    from unittest.mock import MagicMock, patch
    ...
    # Mock API response without the model
    mock_response_data = {
        "data": [
            {"id": "llama-3-8b-instruct", "object": "model"},
    ...
    assert "nonexistent-model" in model_issue
    assert "llama-3-8b-instruct" in model_issue


def test_lm_studio_validate_model_no_models():
    """Test LM Studio model validation when no models are available."""
    import json
    from unittest.mock import MagicMock, patch
    ...
    assert model_valid is False
    assert model_issue is not None
    assert "No models found" in model_issue


def test_lm_studio_validate_model_network_error():
    """Test LM Studio model validation handles network errors gracefully."""
    from unittest.mock import patch
    ...
    # Should assume valid on network errors
    assert model_valid is True
    assert model_issue is None


def test_lm_studio_validate_model_invalid_json():
    """Test LM Studio model validation handles invalid JSON gracefully."""
    from unittest.mock import MagicMock, patch
    ...
    mock_response.__enter__ = MagicMock(return_value=mock_response)
    mock_response.__exit__ = MagicMock(return_value=False)
    mock_response.read.return_value.decode.return_value = "not valid json"

    with patch("urllib.request.urlopen", return_value=mock_response):
        model_valid, model_issue = lm_config._validate_model()

    # Should assume valid on JSON errors
    assert model_valid is True
    assert model_issue is None
...
def test_openai_validate_api_key_valid():
    ...
    assert "api_key_valid" in result.details
...
def test_anthropic_validate_model_recognized():
    """Test Anthropic model validation when model is recognized by LiteLLM."""
    from unittest.mock import MagicMock, patch

    from whai.configuration.user_config import AnthropicConfig
    ...
    # Mock get_model_info to succeed
    mock_model_info = MagicMock()
    with (
        patch("litellm.get_model_info", return_value=mock_model_info),
        patch("whai.configuration.user_config._suppress_stdout_stderr"),
    ):
        model_valid, model_issue = anthropic_config._validate_model()
...
def test_anthropic_validate_model_not_recognized():
    """Test Anthropic model validation when model is not recognized."""
    from unittest.mock import patch
    ...
    anthropic_config = AnthropicConfig(
        api_key="sk-ant-test-key",
        default_model="claude-nonexistent-model",
    )

    # Mock get_model_info to raise exception for unrecognized model
    with (
        patch(
            "litellm.get_model_info",
            side_effect=ValueError("Model 'claude-nonexistent-model' isn't mapped yet"),
        ),
        patch("whai.configuration.user_config._suppress_stdout_stderr"),
    ):
        model_valid, model_issue = anthropic_config._validate_model()
    ...
    assert model_valid is False
    assert model_issue is not None
    assert "claude-nonexistent-model" in model_issue
    assert "not recognized" in model_issue


def test_ollama_validate_api_base_reachable():
    """Test Ollama API base validation when base is reachable."""
    from unittest.mock import MagicMock, patch

    from whai.configuration.user_config import OllamaConfig
    ...
    mock_response = MagicMock()
    with patch("urllib.request.urlopen", return_value=mock_response):
        result = ollama_config.validate()

    # API base should be marked as reachable
    assert result.details.get("api_base_reachable") is True
    assert len([i for i in result.issues if "not reachable" in i]) == 0


def test_ollama_validate_api_base_not_reachable():
    """Test Ollama API base validation when base is not reachable."""
    from unittest.mock import patch
    from urllib.error import URLError
    ...
    # Should have issue about unreachable API base
    assert result.details.get("api_base_reachable") is False
    assert any("not reachable" in issue.lower() for issue in result.issues)


def test_gemini_validate_model_network_error():
    """Test Gemini model validation handles network errors gracefully."""
    from unittest.mock import patch

    from whai.configuration.user_config import GeminiConfig
    ...
    # Mock get_model_info to raise network error
    with (
        patch(
            "litellm.get_model_info",
        ...
        model_valid, model_issue = gemini_config._validate_model()
...
def test_azure_openai_validate_required_fields():
    """Test Azure OpenAI validates all required fields."""
    from whai.configuration.user_config import (
        AzureOpenAIConfig,
    ...
    with pytest.raises(InvalidProviderConfigError, match="requires 'api_key'"):
        AzureOpenAIConfig(
            api_key=None,
    ...
    with pytest.raises(InvalidProviderConfigError, match="requires 'api_base'"):
        AzureOpenAIConfig(
            api_key="test-key",
            api_base=None,
            api_version="2023-05-15",
    ...
    with pytest.raises(InvalidProviderConfigError, match="requires 'api_version'"):
        AzureOpenAIConfig(
            api_key="test-key",
            api_base="https://test.openai.azure.com/",
            api_version=None,
    ...
    with pytest.raises(InvalidProviderConfigError, match="requires 'default_model'"):
        AzureOpenAIConfig(
            api_key="test-key",
            api_base="https://test.openai.azure.com/",
            api_version="2023-05-15",
...
def test_openai_validate_required_fields():
    """Test OpenAI validates required fields."""
    from whai.configuration.user_config import InvalidProviderConfigError, OpenAIConfig

    # Test missing api_key
    with pytest.raises(InvalidProviderConfigError, match="requires 'api_key'"):
        OpenAIConfig(api_key=None, default_model="gpt-5-mini")

    # Test missing default_model
    with pytest.raises(InvalidProviderConfigError, match="requires 'default_model'"):
        OpenAIConfig(api_key="test-key", default_model=None)


def test_ollama_sanitize_model_name():
    """Test Ollama model name formatting for LiteLLM."""
    from whai.configuration.user_config import OllamaConfig
    ...
    # Test _get_litellm_model_name adds prefix
    litellm_name = ollama_config._get_litellm_model_name()
    assert litellm_name == "ollama/mistral"


def test_gemini_sanitize_model_name():
    """Test Gemini model name formatting for LiteLLM."""
    from whai.configuration.user_config import GeminiConfig
    ...
    # Test _get_litellm_model_name adds prefix if missing
    litellm_name = gemini_config._get_litellm_model_name()
    assert litellm_name == "gemini/gemini-2.5-flash"

    # Test with existing prefix (should not add duplicate)
    gemini_config2 = GeminiConfig(
        api_key="AIzaSy-test-key",
        default_model="gemini/gemini-2.5-flash",
    )
    litellm_name2 = gemini_config2._get_litellm_model_name()
    assert litellm_name2 == "gemini/gemini-2.5-flash"
