...
def test_token_truncation_performance(large_text_content):
    ...
    truncated, was_truncated = truncate_text_with_tokens(
        large_text_content, max_tokens=1000
    )
...
def test_large_tmux_pane_capture_performance(monkeypatch):
    ...
    large_tmux_output = "\n".join(
        [f"line {i}: some command output here" for i in range(5000)]
    )
...
You are a helpful terminal assistant.""")
    ...
    assert best_elapsed < 0.02, (
        f"Role loading took {best_elapsed:.3f}s, expected < 0.02s"
    )
...
def test_command_exclusion_from_context_performance(tmp_path, monkeypatch):
    ...
    with patch("whai.utils.detect_shell", return_value="bash"):
        context, _ = get_context(
            exclude_command="whai what is the biggest folder here?"
        )
    elapsed = time.time() - start_time
